// src/test/java/com/example/runners/UiTestRunner.java
package com.example.runners;

import io.cucumber.testng.AbstractTestNGCucumberTests;
import io.cucumber.testng.CucumberOptions;

@CucumberOptions(features = "src/test/resources/features/ui/plantsP", glue = {
        "com.example.ui.stepDefinitions",
        "com.example.hooks"
}, plugin = {
        "pretty",
        "html:target/cucumber-reports/ui/cucumber.html",
        "json:target/cucumber-reports/ui/cucumber.json"
}, monochrome = true, tags = "@TC_UI_PLANT_ADM_005")
public class UiTestRunnerP extends AbstractTestNGCucumberTests {
}